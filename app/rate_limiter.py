...
class TokenBucket:
    ...
    def available(self, client_id: str, now: float | None = None) -> float:
        ...
        # Handle bytes vs string depending on redis client config
        tokens_val = state.get(b"tokens") if b"tokens" in state else state.get("tokens")
        ts_val = state.get(b"ts") if b"ts" in state else state.get("ts")
